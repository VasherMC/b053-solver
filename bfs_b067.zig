...
const Pos = brand.Pos;
const start = brand.b067;
const is_duplicate = brand.is_duplicate_board;

const BT = brand.BT; // backing int of Board
...
fn trace_path(end: Item, last_move: Action, depth: u8, finalized: []const std.ArrayList(Item)) !void {
    ...
    while (d > 0) {
        ...
        const bs = cur.b.reverse_full(cur.p);
        for (bs) |bv| if (bv != Board.invalid) {
            if (bv == start) return;
            const idx = std.sort.binarySearch(Item, finalized[d - 1].items, bv, board_item_cmp);
            if (idx) |i| {
                cur = finalized[d - 1].items[i];
                d -= 1;
                break;
            }
        };
